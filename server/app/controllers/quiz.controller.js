...
exports.create = async (req, res, next) => {
    try {
        const quizService = new QuizService(MongoDB.client);
        const document = await quizService.create(req.body);
...
exports.findAll = async (req, res, next) => {
    ...
    try {
        ...
        if (userName) {
            documents = await quizService.findByName(userName);
        } else {
            documents = await quizService.find({});
        }
    } catch (error) {
